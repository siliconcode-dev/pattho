...
- Sandboxed bypass-permission mode, no command execution outside the workspace folder, git safety on. (2026-09-08: attempted a founder-approved SSH exception for the ingestion VM, but GCE's metadata-based SSH key setup proved persistently broken — reverted to the original rule; commands run on the VM go through the founder pasting into the Console's browser SSH.)
