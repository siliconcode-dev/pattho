...
**Goal:** turn the Physics PDFs (multiple writers, both papers) into searchable, metadata-tagged vectors in Qdrant.
...
- Wire up the GCP VM boot-up → run pipeline → push to Qdrant → spin-down flow end-to-end.
- **Checkpoint:** query Qdrant directly (outside the app) for a known Physics topic and confirm the right chunks, from the right writers, with correct metadata, come back.
...
- Build retrieval: query → embed → Qdrant search → assemble context, respecting the metadata schema (e.g., filter by subject/paper when known).
