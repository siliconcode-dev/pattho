...
- **Vector database:** **Weaviate Cloud** (free tier: 10GB disk, 100k objects, no credit card, genuinely permanent since Oct 2025) — replaced Qdrant Cloud 2026-09-08 once the full Physics corpus's ColBERT multivectors exceeded Qdrant's 4GB free-tier disk. A self-hosted-on-Oracle-Always-Free and a LanceDB-on-object-storage alternative were both tried first and hit dead ends (Oracle ARM capacity shortage; R2 needs a card, B2 lacks required conditional-PUT support, GCS risks real billing). Weaviate natively supports hybrid (dense+BM25) search, so no bring-your-own sparse vectors needed. **ColBERT-style multivector reranking was dropped** — Weaviate's free-tier vector index (`hfresh`) doesn't implement multivector support server-side yet (confirmed with a live error); retrieval is dense+BM25 hybrid only for v1. Revisit if a paid Weaviate tier or self-hosted open-source Weaviate is ever adopted.
